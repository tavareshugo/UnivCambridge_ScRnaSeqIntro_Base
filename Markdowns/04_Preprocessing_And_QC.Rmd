...
We have already prepared a sample
meta data table that relates the sample/run ID to the sample group. The table 
below shows details of the twelve samples that we will be working with.
...
for the purposes of quality assessment we will need to treat them as separate
samples.
...
is set to 1 in all data sets. 

We will now add information from the sample metadata table to the `colData` 
object.
...
About a sixth of the genes have not been detected in any droplet. We can
...
```{r filtering_venns, fig.width=12, fig.height=8}
...
```{r add_per_feature_QC, eval=TRUE}
...
```{r plot_cell_sparsity_histogram}
...
```{r plot_gene_sparsity_histogram}
...
```{r sparsity_gene_filter_results}
table(sparse.genes)
```

Number of cells removed:

```{r sparsity_cell_filter_results}
