...
We will use a data set comprising the 7 samples (1200 cells per sample) analysed with fastMNN.
...
```{r}
# Read object in:
sce <- readRDS("../Robjects/caron_postDeconv_1p2kcps_dsi_PBMMC_ETV6-RUNX1_clust.Rds")
sce
colData(sce) %>% head
```
...
The data set used so far comprises 500 cells per sample, by design, to keep it small
and computations short for the course.
The contingency table above shows some samples contribute few cells to some clusters,
which would prevent their analysis.

For the differential expression and abundance analyses we will use a larger data, with
1200 cells per sample.
We will first load 'uncorrected' (i.e. non merged) SCE object keeping normalised counts.
We then merge samples as done previously, also showing here how to use a convenient function `correctExperiments` from the `batchelor` package, and how to set the order to merge
samples in with `fastMNN`, here PBMMCs sample first, then ETV6-RUNX1s.
We finally perform clustering and dimensionality reduction for visualisation.
